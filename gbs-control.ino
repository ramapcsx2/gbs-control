...
void writeProgramArrayNew(const uint8_t* programArray)
{
  ...
  //writeOneByte(0xF0, 1);
  //writeOneByte(0x1c, 0xff); // in rgbhv preset, might be correct!
  ...
  //writeOneByte(0x17, 0x05); // charge pump current
  //writeOneByte(0x1e, 0x00); // 0x80: decimator will drop data by write enable signal generated by horizontal sync 0x00 write enable is not used.  0x80
...
void loop() {
  ...
  if ((rto->syncWatcher == true) && ((millis() - lastTimeSyncWatcher) > 100)) {
    byte result = getVideoMode();
