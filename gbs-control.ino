...
void writeProgramArrayNew(const uint8_t* programArray)
{
  ...
  // todo: load pll divider section first (tvp7002 manual)
...
void setSPParameters() {
  ...
  writeOneByte(0x35, 0x85); // SP_DLT_REG [7:0]   Sync pulse width difference threshold  (tweak point) (75 for ntsc or pal +10 for master system)
  ...
  writeOneByte(0x38, 0x09); // h coast pre (psx starts "eq or serration" pulses around 4 hsyncs before vs pulse)
  writeOneByte(0x39, 0x09); // h coast post (psx stops "eq or serration" pulses around 4 hsyncs after vs pulse)

  //writeOneByte(0x3e, 0x30); // disable subcoast, enable h-overflow protect
  writeOneByte(0x3e, 0x10); // nope, problems with snes 239 line mode
  ...
  writeOneByte(0x4e, 0x07); writeOneByte(0x4d, 0x00); // test
  writeOneByte(0x50, 0x00); writeOneByte(0x4f, 0x00); // test
...
// -> 10mV per step; recommended 120mV = 0x59 / 0x19
...
boolean inputAndSyncDetect() {
  ...
  writeOneByte(0xF0, 5);
  writeOneByte(0x02, 0x19); // SOG on, slicer level 120mV, input 00 > R0/G0/B0/SOG0 as input (YUV)
  writeOneByte(0xF0, 0);
  ...
  if (syncFound == false) {
    writeOneByte(0xF0, 5);
    writeOneByte(0x02, 0x59); // SOG on, slicer level 120mV, input 01 > R1/G1/B1/SOG1 as input (RGBS)
    writeOneByte(0xF0, 0);
    timeout = 6; // try this input a few times and look for a change
    readFromRegister(0x19, 1, &readout); // in hor. pulse width
    while (timeout-- > 0) {
