...
void writeProgramArrayNew(const uint8_t* programArray)
{
  ...
  //writeOneByte(0x26, 0x05); // was 0x29 // save this in presets, adjusts sampling phase offset

  // same for ADC stuff
  writeOneByte(0xF0, 5);
  setSOGLevel(10);         // 100mV
  writeOneByte(0x18, 0x0); // phase toogle
  //writeOneByte(0x18, 0xa1);// (should be) 180 degrees (tvp7002 default)
  writeOneByte(0x19, 0x0);
  //writeOneByte(0x19, 0xa1);
  writeOneByte(0x1e, 0x00); // 0x80: decimator will drop data by write enable signal generated by horizontal sync 0x00 write enable is not used.
  ...
  //writeOneByte(0x31, 0x00); // 0x1f ntsc vclk
  //writeOneByte(0x32, 0x00); // 0x28 ntsc vclk
...
void setModeDetectParameters() {
  writeOneByte(0xF0, 1);
  writeOneByte(0x60, 0x00);
  writeOneByte(0x61, 0x00);
  writeOneByte(0x82, 0x01);
...
void setSPParameters() {
  ...
  writeOneByte(0x37, 0x58); // SP_H_PULSE_IGNORE (tweak point) H pulse less than this will be ignored. (MD needs > 0x51)
  writeOneByte(0x38, 0x07); // h coast pre (psx starts eq pulses around 4 hsyncs before vs pulse)
  writeOneByte(0x39, 0x01); // h coast post (psx stops eq pulses around 4 hsyncs after vs pulse)

  //writeOneByte(0x3e, 0x30); // disable subcoast, enable h-overflow protect
  writeOneByte(0x3e, 0x00); // nope, problems with snes 239 line mode
  ...
  writeOneByte(0x4e, 0x00); writeOneByte(0x4d, 0x00); // test
  writeOneByte(0x50, 0x06); writeOneByte(0x4f, 0x50); // test
  //writeOneByte(0x4e, 0x00); writeOneByte(0x4d, 0x00);
  //writeOneByte(0x50, 0x00); writeOneByte(0x4f, 0x10);

  writeOneByte(0x55, 0x50); // auto coast off (on = d0, was default)
  writeOneByte(0x56, 0x05); // sog mode on, clamp source 27mhz, no sync inversion (default was invert h sync?)
}
