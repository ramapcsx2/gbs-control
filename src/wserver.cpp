...
static debugRegister * dbgReg = nullptr;
...
void handleSerialCommand()
{
    ...
    case 'D':               // toggle debug mode
        ...
        if (!uopt.debugView)
        {
            dbgReg = new debugRegister();
            dbgReg->HD_Y_OFFSET = GBS::HD_Y_OFFSET::read();
            dbgReg->VDS_Y_OFST = GBS::VDS_Y_OFST::read();
        ...
        else
        {
            ...
            GBS::VDS_Y_OFST::write(dbgReg->VDS_Y_OFST); // restore
            GBS::HD_Y_OFFSET::write(dbgReg->HD_Y_OFFSET);
            if (!rto.inputIsYPbPr)
            {
    ...
    case 'k':               // set OutputRGBHVBypass
        ...
        // slotFlush also does presetSaveToFS()
        slotFlush();
    ...
    case 'T':               // toggle auto gain
        uopt.enableAutoGain = !uopt.enableAutoGain;
...
void handleUserCommand()
{
    ...
    case '7':           // toggle scanlines
        ...
        // slotFlush also does presetSaveToFS()
        slotFlush();
        _WSF(PSTR("scanlines: %s\n"), (uopt.wantScanlines ? F("on (Line Filter recommended)") : F("off")));
        // saveUserPrefs();
        break;
    // case '9':
    //
    ...
    switch (type) {
        ...
        case WStype_PONG:
            if (webSocket.connectedClients() > 0) {
                ...
                constexpr size_t MESSAGE_LEN = 9;
                ...
                // special values
                toSend[8] = rto.videoStandardInput;
